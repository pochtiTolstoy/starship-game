...
GAME_STATES process_help(Render_pipe &rp, UI &ui) {
  ...
  Text_box help_text(rp, HELP_TEXT, color_main_pass, color_shadow_pass, 2, true,
                     0);

  help_text.set_position((SCREEN_WIDTH - help_text.get_width()) / 2,
  ...
  while (state == GAME_STATES::HELP) {
    ...
    while (SDL_PollEvent(&e) != 0) {
      if (e.type == SDL_QUIT)
        ...
        process_help_key(e, state, active_button);
    }
    // Clear screen
    SDL_SetRenderDrawColor(rp.get_renderer(), 0xFF, 0xFF, 0xFF, 0xFF);
    SDL_RenderClear(rp.get_renderer());
