...
GAME_STATES win_page(Render_pipe &rp, UI &ui) {
  ...
  Text_box win_text(rp, "YOU WON!", color_main_pass, color_shadow_pass, 4, true,
                    2);
  win_text.set_position((SCREEN_WIDTH - win_text.get_width()) / 2,
                        (SCREEN_HEIGHT - win_text.get_height()) / 2 - offset);

  Text_box congr_text(rp, "CONGRATULATIONS!", color_main_pass,
                      color_shadow_pass, 4, true, 2);
  congr_text.set_position((SCREEN_WIDTH - congr_text.get_width()) / 2,
  ...
  while (state == GAME_STATES::WIN) {
    ...
    while (SDL_PollEvent(&e) != 0) {
      if (e.type == SDL_QUIT)
        ...
        process_win_key(e, state, active_button);
    }
    // Clear screen
    SDL_SetRenderDrawColor(rp.get_renderer(), 0xFF, 0xFF, 0xFF, 0xFF);
    SDL_RenderClear(rp.get_renderer());
...
void process_win_key(SDL_Event &e, GAME_STATES &state, int &active_button) {
