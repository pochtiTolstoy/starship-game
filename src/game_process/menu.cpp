...
GAME_STATES process_menu(Render_pipe &rp, UI &ui) {
  ...
  // Play button
  Text_box play_button(rp, "PLAY", color_main_pass, color_shadow_pass, 4, false,
                       1);
  play_button.set_position((SCREEN_WIDTH - play_button.get_width()) / 2,
                           (SCREEN_HEIGHT - play_button.get_height()) / 2 -
                               offset);

  // Help button
  Text_box help_button(rp, "HELP", color_main_pass, color_shadow_pass, 4, false,
                       1);
  help_button.set_position((SCREEN_WIDTH - help_button.get_width()) / 2,
  ...
  while (state == GAME_STATES::MENU) {
    ...
    while (SDL_PollEvent(&e) != 0) {
      if (e.type == SDL_QUIT)
        ...
        process_menu_key(e, state, active_button);
    }
    // Clear screen
    SDL_SetRenderDrawColor(rp.get_renderer(), 0xFF, 0xFF, 0xFF, 0xFF);
    SDL_RenderClear(rp.get_renderer());
    ...
    if (prev_button != active_button) {
      ...
      change_active_button_menu(rp, active_button, prev_button, play_button,
...
GAME_STATES get_menu_state(int active_button) {
  ...
  case 0:
    return GAME_STATES::PLAY_MENU;
  case 1:
    return GAME_STATES::HELP;
  case 2:
...
void change_active_button_menu(Render_pipe &rp, int active_button,
                               int prev_button, Text_box &play_button,
                               Text_box &help_button, Text_box &quit_button) {
