...
GAME_STATES process_gameplay1(Render_pipe &rp, UI &ui) {
  ...
  while (!quit && game_is_running(sd, pl)) {
    ...
    // Draw enemy
    for (int i = 0; i < NUM_ENEMY_ON_MAP; ++i) {
      if (meteor_arr[i].detect_planet_collision(pl))
        pl.dec_lifes();
      if (meteor_arr[i].move(delta_time))
        meteor_arr[i].render(rp);
    }
    ...
    for (int i = 0; i < NUM_OBJ_HEALTH_ON_MAP; ++i) {
      ...
      if (obj_health_arr[i].detect_collision(sd))
        add_life(pl, sd);
    }
  ...
  if (sd.kills_ >= KILLS_TO_WIN)
    return GAME_STATES::WIN;
  else
    return GAME_STATES::LVL1_LOSE;
}
...
void process_key(SDL_Event &e, Ship &sd, Enemy *enemy_arr, Orbit &orb) {
  ...
  if (e.type == SDL_KEYDOWN) {
    ...
    case SDLK_w:
      sd.image_ = STATES::MOVE_FORWARD;
      break;
    case SDLK_s:
      sd.image_ = STATES::MOVE_BACKWARD;
      break;
    case SDLK_SPACE:
    ...
    case SDLK_a:
      sd.image_ =
          (sd.image_ == STATES::RELOAD) ? STATES::RELOAD : STATES::DEFAULT;
      break;
    case SDLK_d:
      sd.image_ =
          (sd.image_ == STATES::RELOAD) ? STATES::RELOAD : STATES::DEFAULT;
...
void add_life(Planet &pl, Ship &sd) {
  if (pl.get_curr_lifes() < pl.get_max_lifes())
    pl.inc_lifes();
  else if (sd.curr_lifes_ < sd.max_lifes_)
    ++sd.curr_lifes_;
}
