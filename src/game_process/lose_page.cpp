...
GAME_STATES lose_page(Render_pipe& rp, UI& ui, GAME_STATES curr_lvl) {
  ...
  lose_text.set_position(
    ...
    (SCREEN_HEIGHT - lose_text.get_height()) / 2 - offset - shift_center
  );
  ...
  try_text.set_position(
    ...
    (SCREEN_HEIGHT - try_text.get_height()) / 2 - shift_center
  );

  //Retry button
  Text_box retry_button(
    rp, "RETRY", color_main_pass, color_shadow_pass, 4, false, 1
  );
  retry_button.set_position(
    (SCREEN_WIDTH - retry_button.get_width()) / 2,
    (SCREEN_HEIGHT - retry_button.get_height()) / 2 + shift_button_upper
  );
  ...
  quit_button.set_position(
    ...
    (SCREEN_HEIGHT - quit_button.get_height()) / 2 + shift_button_upper + shift_between_buttons
  );
  ...
  while (state == GAME_STATES::WIN) {
    ...
    if (prev_button != active_button) {
      ...
      switch (prev_button) {
        case -1: break;
        case 0:
          retry_button.update_text(rp, "RETRY", color_main_pass, color_shadow_pass);
          break;
        case 1:
  ...
  if (state == GAME_STATES::RETRY) {
    switch (curr_lvl) {
      case GAME_STATES::LVL1_LOSE: return GAME_STATES::LVL1;
      case GAME_STATES::LVL2_LOSE: return GAME_STATES::LVL2;
      default:
...
GAME_STATES get_state_lose(int active_button) {
  switch (active_button) {
    case 0: return GAME_STATES::RETRY;
    case 1: return GAME_STATES::MENU;
    default:
