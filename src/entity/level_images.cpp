...
void Level_images::render(Render_pipe& rp) {
  for (int i = 0; i < NUM_LEVEL_IMAGES; ++i) {
    level_images_arr_[i].render(rp, x_pos_, y_pos_ + i * (images_gap_ + height_));
  }
}
...
void Level_images::init_images(Render_pipe& rp) {
  ...
  bool success = false;
  for (int i = 0; i < NUM_LEVEL_IMAGES; ++i) {
    if (!level_images_arr_[i].loadFromFile(rp, FILE_PATHS_LEVEL_IMAGES[i], cut_color)) {
...
int Level_images::get_x() const {
  return x_pos_;
}

int Level_images::get_y() const {
  return y_pos_;
}
