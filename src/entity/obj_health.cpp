...
void Obj_health::render(Render_pipe &rp) {
  if (!draw_)
    return;
  // render_.angle += 1;
  gHealthTexture_.render(rp, x_pos_, y_pos_, nullptr, render_);
}
...
bool Obj_health::is_alive() const { return draw_; }

const r_data &Obj_health::get_render_data() const { return render_; }

double Obj_health::get_angle() const { return render_.angle; }

void Obj_health::kill() { draw_ = false; }

int Obj_health::get_y_pos() const { return y_pos_; }

int Obj_health::get_image_height() const { return height_; }
