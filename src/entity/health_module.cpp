...
bool Health_module::render(
  Render_pipe& rp, 
  double delta_time,
  Obj_health oh_arr[]
) {
  if (!alive_ || ind_target_ == -1) return false;
  bool success = false;
...
void Health_module::move_to_heart(double delta_time, Obj_health& oh) {
  ...
  if (!oh.is_alive()) reinit(); //If heart was takes by ship
